...
ZVM_INSTRUCTION_HANDLER_FUNCTION_BEGIN(in)
    ...
    /* ask the device to fill its O[] buffer */
    if(!vm->io_devices[port]->processor(vm, port, function, argc))
        return false;

    /* push the results from device O[] back onto the stack
       (reverse order so the first result ends up on top) */
    if(argc > 0){
