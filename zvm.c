...
void zvm_init_program(zvm_vm_t* vm){
    ...
    /* ── demo: read one character from keyboard, write it to screen ── */

    /* IN keyboard, read_character, argc=1 (one result byte pushed back) */
    LDI(VMP, R0, ZVM_IO_PORT_KEYBOARD)
    LDI(VMP, R1, ZVM_IO_SERVICE_KEYBOARD_READ_CHARACTER)
    LDI(VMP, R2, 1)   /* argc = 1, one result byte will be pushed on stack */

    IN(VMP, R0, R1, R2)

    POP(VMP, R3)      /* the character read is now in R3 */

    /* OUT screen, write_character, argc=1 */
    PUSH(VMP, R3)     /* push the character to send to the device */
